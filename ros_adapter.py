...
if ROS_VERSION in (1,2):
    ...
    SENSOR_TYPES: List[Converter] = [
        ...
        Converter(MagneticField, "magnetic_field", "magnetic", scale=1 / 1_000_000), # microTesla: Tesla
...
class ROS_Adapter:
    def __init__(self, object, rate:float=0.1, name: str = None):
        self.name = name
        self.rate = rate
        self.msgs: Dict[Converter, Publisher] = {}
        for cvt in SENSOR_TYPES:
            if any(hasattr(object, attr) for attr in cvt.source_names):
    ...
    @classmethod
    def create_from_sensor(cls, object):
        if ROS_VERSION==1:
            return ROS1_Adapter(object)
        elif ROS_VERSION==2:
            return ROS2_Adapter(object)
...
class ROS1_Adapter(ROS_Adapter):
    ...
    def __init__(self, object, rate: float = 0.1, name: str = None):
        ...
        super().__init__(object, rate, name)
